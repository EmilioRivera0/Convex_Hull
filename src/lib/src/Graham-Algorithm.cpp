// necessary libraries -------->
#include "../headers/Graham-Algorithm.h"

// functions definition -------->
int dividir(std::vector<Point> &arr, int min, int max)
{
...
void quicksort(std::vector<Point> &arr, int min, int max)
{
    if (min < max)
    {
...
void Min_Y_Coord_Point(std::vector<Point> &puntos) {
	Point min = puntos[0];
	int index = 0;
...
void Order_Angularly(std::vector<Point> &puntos) {
    double x = 0;
    double y = 0;
...
void Graham_Algorithm(std::vector<Point> &puntos, std::stack<Point> &poligono) {
    Point temp;
